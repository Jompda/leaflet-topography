...
import type { TopographyOptions } from './types';
...
async function getTopography(
    latlng: LatLng,
    userOptions?: Partial<TopographyOptions>
) {
    //
    // SETUP:
    ...
    const { tilesUrl, scale, spread, priority, token, retrieveTile, saveTile,
        elevation = true, slope = true, aspect = true, resolution = true } = options;
    ...
    if (elevation) result.elevation = await getElevation({ x: point.x, y: point.y });

    if (slope || aspect || resolution) {
        ...
        if (resolution) result.resolution = (dx + dy) / 2;
        ...
        if (slope || aspect) {
            ...
            if (slope) result.slope = Math.atan(Math.sqrt(dzdx ** 2 + dzdy ** 2)) * (180 / Math.PI);
